        ...
        Expanded(
          child: Padding(
            ...
            child: CatSwipeAnimation(
              onSwipe: (isLike) {
                if (isLike) {
                  context.read<LikedCatsBloc>().add(AddLikedCatEvent(cat));
                }
        ...
        Row(
          ...
          children: [
            ...
            ReactionButton(
              isLike: true,
              onPressed: () {
                final likedTime = DateTime.now().toUtc().add(const Duration(hours: 3));
                final likedCat = cat.copyWith(likedAt: likedTime);
                context.read<LikedCatsBloc>().add(AddLikedCatEvent(likedCat));
                context.read<CatBloc>().add(LoadRandomCatEvent());
              },
              size: 70,
            ),
